...
#import "UIButton+Frequentclicks.h"
#import <objc/runtime.h>
@interface UIButton()
/**bool 类型 YES 不允许点击   NO 允许点击   设置是否执行点UI方法*/
@property (nonatomic, assign) BOOL isIgnoreEvent;
@end
@implementation UIButton (Frequentclicks)
+ (void)load{
...
- (void)mySendAction:(SEL)action to:(id)target forEvent:(UIEvent *)event{
    ...
    if ([NSStringFromClass(self.class) isEqualToString:@"UIButton"]) {
        self.he_timeInterval =self.he_timeInterval == 0 ?defaultInterval:self.he_timeInterval;
        if (self.isIgnoreEvent){
            return;
        }
        if (self.he_timeInterval > 0){
            [[self class] cancelPreviousPerformRequestsWithTarget:self selector:@selector(changeIgnoreState) object:nil];
            [self performSelector:@selector(changeIgnoreState) withObject:nil afterDelay:self.he_timeInterval];
        }
    }
